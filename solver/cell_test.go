...
func TestNewCellConstructFromAnotherCell(t *testing.T) {
	c := NewCellFromIntSlice(0, 0, []int{1, 2, 3, 4})

	c1 := NewCellFromCell(1, 1, *c)
	if !reflect.DeepEqual(c.candidates, c1.candidates) {
		t.Errorf("%v must be equal %v", c.candidates, c1.candidates)
	}
}
...
func TestNewCellConstructFromSliceOfInts(t *testing.T) {
	c := NewCellFromIntSlice(0, 0, []int{1, 2, 3})
...
func TestCellisSolved(t *testing.T) {
	...
	c = NewCellFromIntSlice(0, 0, []int{1, 2})
	if c.isSolved() {
...
func TestExclude(t *testing.T) {
	c := NewCellFromIntSlice(0, 0, []int{1, 2, 3, 4, 5, 6, 7, 8, 9})
